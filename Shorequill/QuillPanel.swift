...
enum QuillPanelSession {
    private static let addressKey = "shorequill.panel.resume.address"
    private static let stampKey = "shorequill.panel.resume.stamp"
...
enum QuillPanelCookies {
    private static let key = "shorequill.panel.cookies"
...
struct QuillWebPanel: UIViewRepresentable {
    ...
    final class Coordinator: NSObject, WKNavigationDelegate {
        ...
        func watchAddress(of webView: WKWebView) {
            ...
            urlObservation = webView.observe(\.url, options: [.new]) { [weak self] webView, _ in
                ...
                QuillPanelSession.remember(webView.url, trackerHost: self.trackerHost)
            }
        }

        func webView(_ webView: WKWebView, didCommit navigation: WKNavigation!) {
            QuillPanelSession.remember(webView.url, trackerHost: trackerHost)
            fire()
        }

        func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
            QuillPanelSession.remember(webView.url, trackerHost: trackerHost)
            QuillPanelCookies.snapshot()
        }
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        QuillPanelCookies.restore { [weak webView] in
...
struct QuillPrivacyRow: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showPolicy) {
            QuillWebPanel(urlString: "https://neonsketch.org/click.php")
        }
    }
}
